...
$(function () {
    ...
    LoadRegionsAutoComplete("#registerRegionSearch", "#registerRegionSearchCheck", "#registerCountry",
        "#registerState", "#registerCity", "#registerLocality");

    LoadRegionsAutoComplete("#mailKitAddressSearch", "#mailKitAddressSearchCheck", "#mailKitAddressCountry",
        "#mailKitAddressState", "#mailKitAddressCity", "#mailKitAddressLocality");
...
submitFormInit = function () {
    ...
    if ($("#termConditions").attr("checked") != "checked") {
        ...
        success: function (response) {
            if (response.isValid == "-1") {
                if (response.errorCode == "ERROR_CAPTCHA") {
                    ...
                    //captcha is wrong
                    $("#idNumberError").show();
                    $("#idNumberError").html("*El número de DNI no fue hallado en nuestros registros, y para este evento es necesario ya estar inscripto.");
                } else {
...
parseIdNumber = function (idNumber) {
    return idNumber.trim().replace(/\./g, "")
        .replace(/\,/g, "")
        .replace(/\-/g, "")
};
        ...
        success: function (response) {
            if (response.isValid == "-1") {
                if (response.errorCode == "ERROR_INVALID_TOKEN") {
                    alert("Ha pasado mucho tiempo y su sesión ha caducado. Porfavor vuelva a ingresar.");
                } else {
                    alert("Se generó un error. Intente más tarde.");
                }
            } else if (response.isValid == "0") {
...
programSelectionHandler = function () {
    ...
    if (EventValues != null && EventValues.length > 0) {
        if (selections.length >= EventValues.length) {
            ...
            //get by qty.
            value = parseFloat(getItemFromList(selections.length, EventValues, "ProgramsQty").Value);
        }
    }
...
submitConfirmPrograms = function () {
    ...
    $(".programSelection").each(function () {
        ...
        if (($.inArray($(this).val(), EnrolledProgramCodes) >= 0 ) || ($.inArray($(this).val(), NotEnrolledProgramCodes) >= 0 )) {
            //show it.
            if ($(this).attr('checked') == 'checked') {
    ...
    if ($("#mailKitSelection").attr('checked') == 'checked') {
        ...
        if (parameter.mailKitAddress.trim() == "") {
            $("#mailKitAddressError").html('Si selecciona envío por correo, la Dirección es obligatoria.');
        ...
        if (parameter.mailKitPostalCode.trim() == "") {
            $("#mailKitPostalCodeError").html('Si selecciona envío por correo, el Código Postal es obligatorio.');
        ...
        if ($("#mailKitAddressSearch").val() == "") {
            $("#mailKitAddressSearchError").html('Si selecciona envío por correo, el "Pais, Estado, Ciudad, Localidad" es obligatorio.');
            $("#mailKitAddressSearchError").show();
            isAnyError = true;
        } else if ($("#mailKitAddressSearch").val() != $("#mailKitAddressSearchCheck").val()) {
            $("#mailKitAddressSearchError").html('El campo "Pais, Estado, Ciudad, Localidad" no es válido. Porfavor revise');
            $("#mailKitAddressSearchError").show();
            isAnyError = true;
        } else {
    ...
    if ($("#otherKitWithdrawnSelection").attr('checked') == 'checked') {
        ...
        if (parameter.otherKitWithdawnName.trim() == "") {
            $("#otherKitWithdawnError").html('Si selecciona Retira otra persona, es necesario especificar el nombre de la persona.');
...
submitRegister = function () {
    ...
    var parameter = {
        ...
        gender: ( $("#registerGenderFemale").attr("checked") == "checked") ? "F" : "M",
...
*/
    if ($("#registerRegionSearch").val() == "") {
        $("#registerRegionError").html('El "Pais, Estado, Ciudad, Localidad" es obligatorio.');
        $("#registerRegionError").show();
        isAnyError = true;
    }

    if ($("#registerRegionSearch").val() != $("#registerRegionSearchCheck").val()) {
        $("#registerRegionError").html('El campo "Pais, Estado, Ciudad, Localidad" no es válido. Porfavor revise');
        $("#registerRegionError").show();
        isAnyError = true;
    }
...
stepConfirmation = function (response) {
    ...
    if (response.value != null) {
		...
		if (response.value.includes("$ ")) {
            ...
            $("#paypalSection").show();
            $("#paypalValue").attr("value", response.value.replace('U$S ','').replace(' Dolares',''));
			$("#paypalDescription").attr("value", response.title);
...
submitConfirmGymName = function () {
    ...
    if (EventType != "LESMILLS_GFM_GERENTE" && EventType != "LESMILLS_FANS" && parameter.responsibleName.trim() == "") {
        $("#confirmGymNameResponsibleError").html('El nombre del responsable es obligatorio');
        $("#confirmGymNameResponsibleError").show();
        isAnyError = true;
    }

    if (EventType != "LESMILLS_GFM_GERENTE" && EventType != "LESMILLS_FANS" && parameter.phone.trim() == "") {
...
confirmGymNameIsAutonomoChange = function () {
    ...
    if ($("#confirmGymNameIsAutonomo").attr('checked') == 'checked') {
...
showNextStep = function (response) {
    ...
    switch (response.nextStep) {
        ...
        case "STEP_REGISTER":
            ...
            $("#registerIdNumberText").val(response.idNumberTypeName + ": " + response.idNumber);
            $("#stepRegister").show();
            break;
