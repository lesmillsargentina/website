...
$token = hash('ripemd160', $_GET['to'] . $_GET['b'] . $_GET['amnt'] . $_GET['v'] . 'lMillsBody#Paypal'); // validamos que venga de body
$domain = "https://bodysystemscontrol.com.ar";
//$domain = "http://local.bodysystemscontrol.com.ar:81";
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-sm-8" style="float:none; margin: 0 auto;">
                ...
                <? if (strtotime($_GET['v']) < strtotime(date('Y-m-d'))) { ?>
                    ...
                    <a href="<?php echo $domain ?>/paymentNotification/paypalContrato?to=<?= $_GET['to'] ?>&billId=<?= $_GET['b'] ?>" class="btn btn-primary btn-block">
                ...
                <? } else { ?>
                    ...
                    // pago parcial
                    if ($_GET["su"] == 0) { ?>
                        <a href="<?php echo $domain ?>/paymentNotification/paypalContrato?to=<?= $_GET['to'] ?>&billId=<?= $_GET['b'] ?>" title="<?= $_GET['desc'] ?>" class="btn btn-primary btn-block">
                            Abonar Licencia $<?= $_GET['amnt'] ?>
                        </a>

                    <? } else { ?>
                        <a href="<?php echo $domain ?>/paymentNotification/paypalSus?to=<?= $_GET['to'] ?>&billId=<?= $_GET['b'] ?>" class="btn btn-primary btn-block">
                        ...
                        <p>
                            ...
                            <a href="<?php echo $domain ?>/paymentNotification/paypalContrato?to=<?= $_GET['to'] ?>&billId=<?= $_GET['b'] ?>" title="<?= $_GET['desc'] ?>" style="color: #000">
                                click aquí
                            </a>.
                        </p>
                    <? } ?>
