...
if (strpos($_SERVER['HTTP_USER_AGENT'], "MercadoPago") !== false && isset($_GET['data_id']) && isset($_GET['type'])) {
	wp_remote_get("http://www.bodysystemscontrol.com.ar/paymentNotification/mercadoPagoIpn?data_id={$_GET['data_id']}&type={$_GET['type']}{$m1}");
    //wp_remote_get("http://staging.bodysystemscontrol.com.ar/paymentNotification/mercadoPagoIpn?data_id={$_GET['data_id']}&type={$_GET['type']}{$m1}");
    //wp_remote_get("http://local.bodysystemscontrol.com.ar:81/paymentNotification/mercadoPagoIpn?data_id={$_GET['data_id']}&type={$_GET['type']}{$m1}");
    error_log("ejecuta a: {$_GET['data_id']}");
    error_log("==========FIN===============");
    die('Ok');
} else if (strpos($_SERVER['HTTP_USER_AGENT'], "MercadoPago") !== false && isset($_GET['id']) && isset($_GET['topic'])) {
	wp_remote_get("http://www.bodysystemscontrol.com.ar/paymentNotification/mercadoPagoIpn?data_id={$_GET['id']}&type={$_GET['topic']}{$m1}");
    //wp_remote_get("http://staging.bodysystemscontrol.com.ar/paymentNotification/mercadoPagoIpn?data_id={$_GET['id']}&type={$_GET['topic']}{$m1}");
    //wp_remote_get("http://local.bodysystemscontrol.com.ar:81/paymentNotification/mercadoPagoIpn?data_id={$_GET['id']}&type={$_GET['topic']}{$m1}");
